...
///
/// Reads the [TodoController] from the ancestor [ChangeNotifierProvider]
/// (provided above the [MaterialApp] navigator in `app.dart`) and composes
...
class _TodoScreenState extends State<TodoScreen> {
  ...
  void initState() {
    super.initState();
    _controller = context.read<TodoController>();

    // Trigger data load if not already loaded.
    if (_controller.allTasks.isEmpty && !_controller.isLoading) {
  ...
  void _navigateToEditTask(Task task) {
    Navigator.of(
      context,
    ).pushNamed(Routes.taskEditPath(task.id), arguments: task);
  }
...
class _TodoScreenContent extends StatefulWidget {
  ...
  @override
  State<_TodoScreenContent> createState() => _TodoScreenContentState();
}
              ...
              children: [
                ...
                Padding(
                  ...
                  child: FilterBar(
                    ...
                    activeCategoryId: controller.categoryFilter,
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      const Spacer(),
                      SortControl(
                        activeCriterion: controller.sortCriterion,
                        activeDirection: controller.sortDirection,
                        onCriterionChanged: controller.setSortCriterion,
                        onDirectionChanged: controller.setSortDirection,
                      ),
                    ],
                  ),
                ),
